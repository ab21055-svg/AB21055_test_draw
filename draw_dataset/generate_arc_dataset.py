...
def main() -> None:
    ...
    params = ArcGenerator(drawing_size=DRAWING_SIZE).gen_ent(
        COUNT,
        min_radius=10.0,
        min_arc_angle=10.0,
        max_radius=158.0,
        min_rendered_length=30.0,
    )
    Arc2Img(drawing_size=DRAWING_SIZE, img_size=IMG_SIZE).draw_imgs(params, str(dataset_dir))

    raw_rows = []
    label_rows = []
    for index, row in enumerate(params):
        image_name = f'p{index:05d}.jpg'
        raw_rows.append([image_name, *map(float, row), 0.0])
        label_rows.append([
            image_name,
            float(row[8]),
            float(row[9]),
    ...
    write_csv(
        ...
        ['image', 'line_type', 'pad1', 'pad2', 'pad3', 'pad4', 'pad5', 'pad6', 'pad7', 'center_x', 'center_y', 'radius', 'start_angle', 'cw_sweep_angle', 'label'],
        raw_rows,
    )
    write_csv(
        dataset_dir / 'labels.csv',
        ['image', 'center_x', 'center_y', 'radius', 'start_angle', 'cw_sweep_angle'],
        label_rows,
    )

    print('Generated arc dataset:', dataset_dir)
    print('Count:', COUNT)
